# @womorg/reimagined-enigma

Parse JSON without risk of losing numeric information.

```js
import { parse, stringify } from '@womorg/reimagined-enigma'
...
**How does it work?** The library works exactly the same as the native `JSON.parse` and `JSON.stringify`. The difference is that `@womorg/reimagined-enigma` preserves information of big numbers. `@womorg/reimagined-enigma` parses numeric values not as a regular number but as a `LosslessNumber`, a lightweight class which stores the numeric value as a string. One can perform regular operations with a `LosslessNumber`, and it will throw an error when this would result in losing information.

**When to use?** If you have to deal with JSON data that contains `long` values for example, coming from an application like C++, Java, or C#. The trade-off is that `@womorg/reimagined-enigma` is slower than the native `JSON.parse` and `JSON.stringify` functions, so be careful when performance is a bottleneck for you.
...
Install via [npm](https://www.npmjs.com/package/@womorg/reimagined-enigma):

```
npm install @womorg/reimagined-enigma
```
...
```js
import { parse } from '@womorg/reimagined-enigma'
...
```js
import { parse, isInteger } from '@womorg/reimagined-enigma'
...
```js
import { parse, isSafeNumber } from '@womorg/reimagined-enigma'
...
- `reviveDate(key, value)`
  ...
  ```js
  import { parse, reviveDate } from '@womorg/reimagined-enigma'
...
(Spoiler: `@womorg/reimagined-enigma` is much slower than native)
